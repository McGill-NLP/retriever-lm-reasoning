...
def gen_ctx_vectors(
    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        # TODO Parishad remove
        if total % 100 == 0:
            break
    return results


@hydra.main(config_path="../conf/dpr", config_name="gen_embs", version_base="1.1")
def main(cfg: DictConfig):
    ...
    # TODO Parishad remove
    print('shard_passages', len(shard_passages), shard_passages[:1])

    data = gen_ctx_vectors(cfg, shard_passages, encoder, tensorizer, True)
    print('vectors', len(data), data[0][1].shape)
...
# python -m DPR.generate_dense_embeddings model_file=/network/scratch/p/parishad.behnamghader/FiD/DPR/dpr/downloads/checkpoint/retriever/single/nq/bert-base-encoder.cp ctx_src=dpr_wiki shard_id=0 num_shards=1 out_file=my_out_file