...
def init_distributed_mode(params):
    ...
    # SLURM job
    # print(os.environ)
    # print('SLURM_JOB_ID' in os.environ, 'SLURM_PROCID' in os.environ, os.environ['SLURM_JOB_ID'])
    if params.is_slurm_job and has_local_rank:

        assert params.local_rank == -1   # on the cluster, this is handled by SLURM

        # original: SLURM_NTASKS --> SLURM_STEP_NUM_TASKS
        SLURM_VARIABLES = [
            'SLURM_JOB_ID',
            'SLURM_JOB_NODELIST', 'SLURM_JOB_NUM_NODES', 'SLURM_STEP_NUM_TASKS', 'SLURM_TASKS_PER_NODE',
        ...
        params.node_id = int(os.environ.get('SLURM_NODEID', 1))

        # local rank on the current node / global rank
        params.local_rank = int(os.environ.get('SLURM_LOCALID', 1))
        params.global_rank = int(os.environ.get('SLURM_PROCID', 1))

        # number of processes / GPUs per node
        params.world_size = int(os.environ.get('SLURM_STEP_NUM_TASKS', '1'))
        params.n_gpu_per_node = params.world_size // params.n_nodes

        # define master address and master port
        hostnames = subprocess.check_output(['scontrol', 'show', 'hostnames', os.environ.get('SLURM_JOB_NODELIST', '1')])
        ...
        params.is_distributed = params.n_gpu_per_node > 1
