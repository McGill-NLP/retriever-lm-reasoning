...
class SequenceScorer(object):
    ...
    def generate(self, models, sample, **kwargs):
        ...
        for model in models:
            model.eval()
            # net_input = {'src_tokens': torch.tensor([[ 1699, 12531,   211,    50,     4,    88, 12328,    10,   918,     6]]).cuda(),
            #              'src_lengths': torch.tensor([10]).cuda()}
            ...
            for i, (bd, tgt, is_single) in enumerate(batched):
                ...
                # print(model)
                # print(curr_prob.sum(-1), curr_prob.shape)  # curr_prob size [1, n, 267744]
                # TODO remove
                # curr_prob_ = torch.where(curr_prob.lt(0), curr_prob, torch.tensor(-torch.inf).half().cuda())
                # best_pred = torch.argmax(curr_prob_[0, :, :], dim=-1)
                # print('net', ' '.join([task.source_dictionary[w_ind] for w_ind in net_input['src_tokens'][0]]))
                # print(best_pred.shape, ' '.join([task.source_dictionary[w_ind] for w_ind in best_pred]), curr_prob.shape)
        ...
        # print('scorer', decoder_out[1][self.args.knn_keytype].shape)
        return hypos
