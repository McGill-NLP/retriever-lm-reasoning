...
class RealmReader(RealmPreTrainedModel):
    ...
    def forward(
        ...
        # print('reader beam', self.config.reader_beam_size,
        #       'reader logit', reader_logits.shape,
        #       'ret logit', retriever_logits.shape)
        # []
        ...
        # print('cands logit', torch.max(reader_logits, dim=1).values, 'best', predicted_block_index)
        # [1]
...
class RealmForOpenQA(RealmPreTrainedModel):
    ...
    def forward(
        ...
        # k_min = min(k, self.block_emb.shape[0])
        _, retrieved_block_ids = torch.topk(batch_scores, k=min(self.config.searcher_beam_size, self.block_emb.shape[0]), dim=-1)  # TODO Parishad
        # print(retrieved_block_ids.shape)
        # [searcher_beam_size]
        retrieved_block_ids = retrieved_block_ids.squeeze()
        # print('retrieved ids', retrieved_block_ids)  # TODO Parishad
        ...
        # print('beam size', self.config.reader_beam_size)
        # reader_output = self.reader(
        #     input_ids=concat_inputs.input_ids[0: k_min],
        #     attention_mask=concat_inputs.attention_mask[0: k_min],
        #     token_type_ids=concat_inputs.token_type_ids[0: k_min],
        ...
        if not return_dict:
            return reader_output, predicted_answer_ids, concat_inputs.input_ids, retrieved_block_ids

        # return RealmForOpenQAOutput(
        #     reader_output=reader_output,
        #     predicted_answer_ids=predicted_answer_ids,
        #     retrieved_blocks_ids=concat_inputs.input_ids
        # )
        return RealmForOpenQAOutput(
            reader_output=reader_output,
            predicted_answer_ids=predicted_answer_ids,
